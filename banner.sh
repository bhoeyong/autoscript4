...
[[ "$(echo "$chk" | grep -v '#Banner' | grep Banner)" != "" ]] && {
    ...
    local="/etc/issue.net"
    [[ $(grep -wc $local /etc/ssh/sshd_config) = '0' ]] && echo "Banner /etc/issue.net" >> /etc/ssh/sshd_config
}
...
echo ""
echo -ne "\033[1;32mChoose number\033[1;31m ?\033[1;37m : "; read resp
if [[ "$resp" = "1" ]]; then
echo ""
echo -ne "\033[1;32mWhich message do you want to Display\033[1;31m ?\033[1;37m : "; read msg1
if [[ -z "$msg1" ]]; then
...
echo -e "\n\033[1;32mBANNER CHANGED SUCCESSFULLY !\033[0m"
sleep 2
menu
else
